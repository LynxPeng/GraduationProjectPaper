...
其中为提高性能，查询首个零位使用Rust内建方法 \verb|trailing_zeros|\cite{Lin2019RustRiscvSIMD}。该方法返回最低有效零位的索引，在risc-v中可以被映射成CLZ等硬件指令\cite{Waterman2014RISCVISA}。具体算法如算法\ref{alg:find_first_unset}所示。
...
\begin{algorithm}
  ...
  \begin{algorithmic}
    \Require input：bitmap（32位整数）
    \Ensure output：首个零位的索引（从最低位开始计数），若无零位则返回None
...
该算法输入为一个32位整数，表示当前中断向量的使用情况。输出为首个未使用的中断向量号索引，若无可用向量则返回None。该算法首先对输入的位图进行按位取反，得到未使用的中断向量的位图。若该位图为0，则表示所有中断向量均已被使用，返回None。否则，使用Rust内建的\verb|trailing_zeros|方法查询最低有效零位的索引，并返回该索引作为可用中断向量号。
...
\begin{figure}[htbp]
  \centering
  \includesvg[inkscapelatex=false,width=0.8\textwidth]{images/IrqAllocator_v2}
...
为了在通信时附带协程所申请的中断向量信息，本设计在\texttt{IPC\_Item}中新增了vec字段用于指示发送消息的协程的中断向量。若该字段为0，则表明该协程无中断向量，需通过\texttt{dispatcher}协程唤醒。修改后的IPC\_Item结构如图\ref{ipc_item结构}。

% ipcitem位域图
\begin{figure}[htbp]
  \centering
  \includesvg[inkscapelatex=false,width=\textwidth]{images/ipc_item}
  \caption{IPC\_Item位域图}\label{ipc_item结构}
\end{figure}
...
\begin{figure}[htbp]
  \centering
  \includesvg[inkscapelatex=false,width=0.9\textwidth]{images/newbuffer_v2}
...
\begin{algorithm}
  \caption{客户端异步系统调用请求}\label{alg:client_call}
  \begin{algorithmic}
    ...
    \State $reply \gets$ \Call{riscv\_Page\_Map}{vec, args} \Comment{异步等待}
...
\begin{algorithm}
  \caption{异步过程调用发送函数}\label{syscallsend}
...
\Function{async\_syscall\_handler}{$process\_id$, $tcb$}
    ...
    \Loop
        ...
        \Else
            ...
            \State \Call{yield\_now}{} \Comment{阻塞等待通知}
        \EndIf
    \EndLoop
\EndFunction
\end{algorithmic}
