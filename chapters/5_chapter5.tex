...
\begin{figure}[htbp]
  ...
  \includesvg[inkscapelatex=false,width=\textwidth]{images/syscall_test_zn_v2}
...
\begin{figure}[htbp]
  \centering
    % \includepdf[fitpaper]{images/syscall_test.pdf}
  \includesvg[inkscapelatex=false,width=\textwidth]{images/syscall_test_zn_hcon}
  \caption{高并发下的异步系统调用性能}\label{性能测试结果2}
\end{figure}

图\ref{性能测试结果2}给出了在高并发下的异步系统调用的性能测试结果。可以看出，在并发度为64以上时，所有方案的平均用时均趋于平稳。使用TAIC优化后的异步系统调用平均用时仍低于原有实现，但随并发度的增加，优化后的平均用时变化较小。一方面，TAIC主要针对低并发下无法被均摊的切换开销进行优化；在高并发下，异步系统调用的平均用时主要受限于内核处理的速度。另一方面，TAIC中断向量的分配与回收开销较小，不会因并发度的增加而显著影响平均用时。
...
图\ref{性能测试结果}中折线图部分展示了系统调用的陷入频率。

实验结果表明，使用TAIC优化后的异步系统调用陷入频率随并发度的升高而下降。如图中所示，在单核环境下，异步系统调用的陷入频率从初始并发度1开始，逐步下降至并发度32时的0.08；而在多核环境下该频率与单核变化趋势类似，但在并发度大于8时较单核略有提高。
...
性能测试中，对内存映射与解除映射的异步系统调用，在不同并发度、不同核心下的平均用时和陷入频率进行了测试评估。测试结果显示，TAIC优化在不同并发度下均有效降低了系统调用的平均用时和陷入频率，尤其在低并发和多核场景中优势显著，相较原有ReL4系统具有明显性能提升。\cite{lzb1,lzb2,lzb3,lzb4,lzb5,lzb6,lzb7,lzb8}
