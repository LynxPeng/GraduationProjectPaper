...
\begin{figure}[htbp]
    \centering
    \includesvg[inkscapelatex=false,width=0.8\textwidth]{images/async_runtime_v2}
    \caption{Rust语言异步运行时框架图}\label{asyncruntime}
\end{figure}

Rust的异步运行时围绕 Executor和Reactor设计。Executor是一个调度器，负责管理和调度异步任务的执行。Reactor是一个事件循环，负责等待IO事件的发生，并将事件通知到对应的异步任务。

图\ref{asyncruntime}给出了异步运行时中Executor和Reactor的结构框架。
...
ReL4采用与seL4相似的能力安全模型作为其访问控制体系。在该模型下，可被用户态线程使用的系统资源被抽象为内核对象。线程访问对应的内核对象需要具有相应的能力并提供能力句柄\cite{2008Verified}。ReL4中的主要内核对象包括：TCB、CSpace、VSpace、Notification、Endpoint、Untyped和Frame。
...
\begin{figure}[htbp]
    ...
    \caption{协程调度器结构示意图}\label{Executor}
\end{figure}
...
\begin{figure}[htbp]
    \centering
    \includesvg[inkscapelatex=false,width=\textwidth]{images/async_ipc_v2}
...
\begin{figure}[htbp]
    \centering
    \includesvg[inkscapelatex=false,width=\textwidth]{images/taic_arch_v2.svg}
